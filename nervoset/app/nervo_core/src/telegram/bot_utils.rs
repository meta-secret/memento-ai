...
pub async fn chat(bot: Bot, msg: Message, app_state: Arc<AppState>) -> anyhow::Result<()> {
    ...
    if matches!(&msg.chat.kind, ChatKind::Private(_)) || is_reply || (matches!(&msg.chat.kind, ChatKind::Public(_)) && is_text) {
...
pub async fn system_message(bot: &Bot, msg: &Message, message_type: SystemMessage) -> anyhow::Result<()> {
...
async fn save_user_id(app_state: Arc<AppState>, user_id: String) -> anyhow::Result<()> {
    ...
    if !user_ids.contains(&user_id) {
        ...
        app_state.local_db.save_message(user, "all_users_list", false).await?;
    }
    Ok(())
}
